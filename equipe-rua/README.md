...
estrutura de navegação (sidebar + barra inferior no celular, tema claro/escuro) dos outros
painéis deste repositório — inclusive o mesmo Supabase usado no `painel-captacao`.
...
rua. Sem etapa de build, basta subir a pasta em qualquer hospedagem estática (ou até abrir com
um servidor local) — não precisa de Node, `npm install` nem pipeline de deploy.
