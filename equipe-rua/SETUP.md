...
precisa escrever código. No fim, os únicos valores que você vai colar num arquivo são os que
o próprio Firebase te entrega prontos, copiando e colando.

---

## 1. Criar o projeto no Firebase

1. Acesse **https://console.firebase.google.com** numa aba nova e faça login com sua conta
   Google (pode ser a mesma do Gmail).
2. Clique em **Criar um projeto** (ou **Adicionar projeto**).
3. Dê um nome, por exemplo `equipe-rua-estefane`. Clique em **Continuar**.
4. Pode desmarcar o Google Analytics (não é necessário para este app). Clique em **Criar
   projeto** e aguarde alguns segundos.
...
4. Em modo de segurança, escolha **Iniciar no modo de teste** (regras abertas por 30 dias —
   depois disso, se quiser manter aberto, é só entrar em **Regras** e clicar em **Publicar**
   de novo para renovar o prazo; ou usar as regras da seção 4 abaixo, que não expiram).
5. Clique em **Ativar**.
...
5. Abra o arquivo `equipe-rua/js/firebase-config.js` deste projeto e substitua cada
   `COLE_AQUI_...` pelo valor correspondente que você copiou. Salve o arquivo.

   Exemplo de como fica depois de preenchido:
   ```js
   export const firebaseConfig = {
     apiKey: 'AIzaSyD...',
     authDomain: 'equipe-rua-estefane.firebaseapp.com',
     projectId: 'equipe-rua-estefane',
     storageBucket: 'equipe-rua-estefane.appspot.com',
...
- **Mais simples**: publique a pasta `equipe-rua/` em qualquer hospedagem de arquivos
  estáticos (GitHub Pages, Netlify, Firebase Hosting, etc.) e acesse o link com o celular ou
  computador — é só HTML/CSS/JS puro, sem instalação, sem build.
- **Testar localmente**: dentro da pasta `equipe-rua/`, rode `python3 -m http.server 8080` (ou
