...
export default function ProjetosLei() {
  ...
  const [janelaAtualizacoes, setJanelaAtualizacoes] = useState(30);

  const projetos = store.projetos;

  const atualizacoesFiltradas = useMemo(() => {
    const todas = store.projetosFonte?.atualizacoesRecentes || [];
    const corte = new Date();
    corte.setDate(corte.getDate() - janelaAtualizacoes);
    const corteISO = corte.toISOString().slice(0, 10);
    return todas.filter((a) => a.data >= corteISO);
  }, [store.projetosFonte?.atualizacoesRecentes, janelaAtualizacoes]);
        ...
        <ScrollReveal delay={0.05} className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-1.5">
              {[
                ...
                <button
                  key={op.dias}
                  onClick={() => setJanelaAtualizacoes(op.dias)}
                  className={`rounded-full px-3 py-1 text-xs font-medium transition ${
                    janelaAtualizacoes === op.dias
          ...
          <div className="mt-3 divide-y divide-slate-100 dark:divide-slate-800">
            ...
            {atualizacoesFiltradas.length === 0 && (
              <p className="py-4 text-center text-sm text-slate-400">Nenhuma atualização nos últimos {janelaAtualizacoes} dias.</p>
            )}
          </div>
        </ScrollReveal>
      )}
