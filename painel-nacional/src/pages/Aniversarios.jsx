...
import { LuCake, LuFlame, LuPartyPopper } from 'react-icons/lu';
...
function CardAniversario({ p, ehHoje }) {
  return (
    ...
    >
      <span className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold ${ehHoje ? 'bg-amber-500 text-white' : 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300'}`}>
        {ehHoje ? (<><LuPartyPopper className="h-3 w-3" /> Hoje</>) : String(p.dm.dia).padStart(2, '0')}
      </span>
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 pb-24 sm:px-6 lg:px-10 lg:pb-8">
      <ScrollReveal>
        <h1 className="flex items-center gap-2.5 text-2xl font-semibold text-slate-900 dark:text-white">
          <LuCake className="h-6 w-6 text-indigo-500" /> Aniversários
        </h1>
      ...
      {aniversariantesHoje.length > 0 && (
        <ScrollReveal delay={0.06} className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/40 dark:bg-amber-500/10">
          <p className="flex items-center gap-2 text-sm font-semibold text-amber-800 dark:text-amber-300">
            <LuPartyPopper className="h-4 w-4" /> Aniversariante(s) de hoje
          </p>
