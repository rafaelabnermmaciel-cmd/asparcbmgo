...
// Grade de campos compartilhada entre o formulário de novo cadastro e a edição inline de um
// já existente — só o que envolve anexos/envio fica de fora daqui (cada chamador cuida disso).
function CamposCaptacao({ valores, onChange, quarteis, militares, nomesParlamentares }) {
...
function paraPayload(f, quarteis) {
...
// pessoa clica em "Editar". Não mexe em anexos (só o cadastro inicial anexa arquivo).
function EdicaoCaptacao({ captacao, quarteis, militares, nomesParlamentares, onSalvar, onCancelar }) {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      await onSalvar(paraPayload(f, quarteis));
    } catch (err) {
...
export default function Cadastro() {
  ...
  async function handleSubmit(e) {
    ...
    setEnviando(true);
    try {
      await submitCaptacao({ ...paraPayload(f, quarteis), anexos });
